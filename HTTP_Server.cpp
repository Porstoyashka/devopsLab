...
#include <chrono>
#include <algorithm>
#include "FuncA.h"  // Include FuncA header
                ...
                else if(!strcmp(strHTTP_requestPath, "/compute"))
{
    ...
    // Use FuncA to calculate values instead of sorting
    double result = 0.0;
    
    // Increase the number of outer loop iterations from 500 to 2000
    for (int i=0; i<2000; i++) {
        // Process more elements by changing the modulo check
        for (size_t j=0; j<aValues.size(); j++) {
            // Check every 100th element instead of every 1000th (10x more elements)
            if (j % 100 == 0) {
                // Use a larger input for calculation (up to 50 instead of 20)
    ...
    // Create a result string including both time and calculated value
    char strTimeEllapsed[50];
    sprintf(strTimeEllapsed, "Time: %i ms, Result: %.2f", iMS, result);
